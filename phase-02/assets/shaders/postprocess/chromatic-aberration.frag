...
void main(){
    ...
    vec4 sample = texture(tex, tex_coord);

    vec2 moveLeft = vec2(tex_coord.x - STRENGTH, tex_coord.y);
    vec4 leftSample = texture(tex, moveLeft);

    vec2 moveRight = vec2(tex_coord.x + STRENGTH, tex_coord.y);
    vec4 rightSample = texture(tex, moveRight);

    frag_color = vec4(leftSample.x, sample.y, rightSample.z, sample.w);
